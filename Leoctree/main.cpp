...
void leer()
{
	...
	while (!file.eof())
	{
		...
		file >> y;
		file >> z;
		arr.push_back(pointOctree(x*10 , y*10 , z*10 ));
	}
}

Octree *octavius = new Octree(pointOctree(-20, 20), pointOctree(-20, 20), pointOctree(20, -20), 4000000);
...
int main(int argc, char** argv) 
{
	...
	//octavius->insertar(pointOctree(2, 1, 2));
	//octavius->insertar(pointOctree(-3, -3, -3));
	//octavius->insertar(pointOctree(-2, -2, -2));
	//octavius->insertar(pointOctree(-2, -1, -2));
	leer();
